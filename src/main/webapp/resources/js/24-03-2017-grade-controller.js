...
productApp.controller('gradeController', ['$rootScope','$scope','$http','$filter', function($rootScope,$scope,$http,$filter) {
	...
	$scope.getSelectedProduct=function(selectedProduct){
		...
		if(selectedProduct.productName.toUpperCase()=='HARDWARE'){
	...
	$scope.refererAgents.selected = "NO AGENT";
		...
		.success(function(data) {
			...
			if(data.object.length>0){
				$.each(data.object,function(k,val){
					$scope.refererAgents.values.push(val);
				});
				$scope.refererAgents.values.push(agentObject);
			}
		});
	}
	...
	$scope.getAvailableStock=function(selectedGrade){
		$scope.selectedGrade={};
	...
	$scope.totalQuantity;
	$scope.validateTotalGrade=function(selectedGrade, Id){
		 if(selectedGrade.newQuantity > $scope.selectedGrade.totalQuantityArrived){
			$('#warehouseMsg'+Id).text('Please enter the valid quantity less than Total quantity arrived');
			$('#warehouseMsg'+Id).show();
		}else{
			$('#warehouseMsg'+Id).hide();
	...
	$scope.addGrade = function() {
		   ...
		   if(category.toUpperCase()=='VENDORCATEGORY'){
			...
			if(!itemExists){
				...
				if(orderProduct.sizeUnit=="MTR"){
				 ...
				 orderItem = {
						...
						//	gradeStockInfo: selectedGrade.warehouseBreakups,
							totalPrice: $scope.getItemTotalPrice(orderProduct),
				...
				if(mode.toUpperCase()=='ADD'){
					...
					orderItem.gradeStockInfo=orderProduct.warehouseBreakups;
					$scope.orderItems.push(orderItem);
				}else if(mode.toUpperCase()=='EDIT'){
					orderItem.isNew=false;
					orderItem.warehouseBreakups=orderProduct.warehouseBreakups;
					orderItem.gradeDetail=$scope.productVendor.selectedProduct.productName +"-"+ $scope.productVendor.selectedVendor.name + "-"+orderProduct.gradeDetails,
	   ...
	   $scope.getHardwareTotalPrice=function(item,from){
		   ...
		   if(from=='ADD'){
				  ...
				  if(val.hardwareStockId==item.gradeId){
					  var hardwareItemTotalPrice=val.quantity *  val.unitPrice;
					  totalPrice=hardwareItemTotalPrice.toFixed(2);
					  val.totalPrice=totalPrice;
					 // $scope.getGrandTotal('ADD');
					  return false;
	...
	$scope.verifyOrder=function(orderForm){
		...
		if(inValidParams.length==0){
			...
			console.log($scope.orderItems);
			var orderList=[];
			if($scope.orderItems.length>0 && $scope.totalOrderAmount>0){
				$.each($scope.orderItems,function(key,val){
					orderList.push({
						itemId:"",
						gradeId:val.gradeId,
				...
				var orderDetail={
						...
						itemList:orderList,
						orderAmount:parseFloat($scope.grandTotal)
	...
	$scope.searchOrder=function(){
		...
		.success(function(data){
			...
			$scope.editedOrder.orderAmount=data.object.orderAmount;
			 angular.copy($scope.editedOrder, $scope.clonedOrder);
			var orders=[];
			if(data.statusCode==200){
